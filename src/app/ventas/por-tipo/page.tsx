...
export default function VentasPorTipoPage() {
  ...
  const openJugadores = useCallback(async (d: DetalleRow, tipo: TipoRow, groupBy: "mes" | "producto") => {
    ...
    try {
      const params = new URLSearchParams({ dateFrom, dateTo, idTipoProducto: String(tipo.IdTipoProducto) });
      if (idSede !== "all") params.set("idSede", String(idSede));
      if (groupBy === "mes") params.set("mes", String(d.Mes));
      else if (d.IdProducto != null) params.set("idProducto", String(d.IdProducto));
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                  ...
                  <div className="grid grid-cols-[1fr_auto_auto] gap-3 px-4 py-3 border-t border-white/10 bg-white/5">
                ...
                <p className="text-[11px] text-slate-500 mt-2">Haz clic en un rectángulo o en una fila para ver el detalle.</p>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[130] p-4">
            <div className="bg-[#0f172a] border border-white/15 rounded-3xl w-full max-w-4xl max-h-[88vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
              ...
              <div className="p-4 bg-white/5 border-t border-white/10 flex justify-between items-center text-[11px] text-slate-500 px-6">
                <p>{jugData.length} venta(s)</p>
                <p className="font-black text-white">
                  Total: <span className="text-emerald-400">{fmt2(jugData.reduce((s, j) => s + j.Pago, 0))}</span>
                </p>
              </div>
            </div>
          </div>
