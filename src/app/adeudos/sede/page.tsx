...
import { Search, MapPin, UserMinus, UserCheck, CheckCircle2, ChevronRight } from 'lucide-react';
...
export default function AdeudosSedePage() {
  ...
  const fetchSedes = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/adeudos/sedes');
            ...
            <div className="flex gap-4 w-full md:w-auto">
              <div className="flex-1 md:flex-none bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 rounded-xl flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">{sedes.reduce((acc, curr) => acc + curr.Activos, 0)}</p>
                </div>
              </div>
      ...
      <div className="p-5 block relative z-10 h-full">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            <div className={`p-2 rounded-lg border transition-colors ${sede.PendientesInscripcion > 0 ? 'bg-amber-500/5 border-amber-500/10 text-amber-400/80' : 'bg-emerald-500/5 border-emerald-500/10 text-emerald-400/80'}`}>
              <p className="text-[7px] uppercase font-black mb-0.5 tracking-tighter opacity-60">Inscripción</p>
              <p className="text-[10px] font-black">{sede.PendientesInscripcion > 0 ? `${sede.PendientesInscripcion} Pend.` : '✓ OK'}</p>
            </div>
            <div className={`p-2 rounded-lg border transition-colors ${sede.PendientesMensualidad > 0 ? 'bg-rose-500/5 border-rose-500/10 text-rose-400/80' : 'bg-emerald-500/5 border-emerald-500/10 text-emerald-400/80'}`}>
              <p className="text-[7px] uppercase font-black mb-0.5 tracking-tighter opacity-60">Mensualidad</p>
              <p className="text-[10px] font-black">{sede.PendientesMensualidad > 0 ? `${sede.PendientesMensualidad} Pend.` : '✓ OK'}</p>
            </div>
          </div>
        </div>
