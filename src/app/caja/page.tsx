...
export default function CajaPage() {
  ...
  const fetchAperturas = useCallback(async (p: Period, from: string, to: string) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const applyCustomDates = () => {
    ...
    fetchAperturas("custom", pendingFrom, pendingTo);
  };

  // Group by sede, preserving insertion order (already sorted by API)
  const sedeMap = new Map<number, { sede: string; items: Apertura[] }>();
  for (const ap of aperturas) {
    if (!sedeMap.has(ap.IdSede)) sedeMap.set(ap.IdSede, { sede: ap.Sede, items: [] });
    sedeMap.get(ap.IdSede)!.items.push(ap);
  }
  const sedeEntries = [...sedeMap.entries()];
  ...
  const totalVentas  = aperturas.reduce((s, a) => s + Number(a.TotalVentas),  0);
  const totalEgresos = aperturas.reduce((s, a) => s + Number(a.TotalEgresos), 0);
  const openCount    = aperturas.filter((a) => !a.FechaCierre).length;
      ...
      <main className="overflow-y-auto flex-1 text-white">
        ...
        <div className="bg-white/5 backdrop-blur-xl border-b border-white/10 px-6 py-4 flex justify-between items-center sticky top-0 z-20">
          <div>
            ...
            <p className="text-xs text-blue-300 mt-0.5">Aperturas y cortes por sede — {periodLabel}</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => fetchAperturas(period, dateFrom, dateTo)}
              disabled={isLoading}
        ...
        <div className="p-6 md:p-8 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h2 className="text-lg font-black">Aperturas de Caja</h2>
              <p className="text-xs text-slate-400 mt-0.5">Turnos registrados por sede — {periodLabel}</p>
            </div>
          ...
          {!isLoading && aperturas.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: "Total Sedes",     value: sedeEntries.length.toString(),       sub: "con aperturas",      icon: <MapPin    size={16} className="text-blue-400"   />, ibg: "bg-blue-500/10 border-blue-500/20"    },
                { label: "Total Aperturas", value: aperturas.length.toString(),          sub: `${openCount} en curso`, icon: <KeyRound  size={16} className="text-emerald-400"/>, ibg: "bg-emerald-500/10 border-emerald-500/20" },
                { label: "Total Ventas",    value: fmt(totalVentas),                     sub: "ingresos del período", icon: <DollarSign size={16} className="text-emerald-400"/>, ibg: "bg-emerald-500/10 border-emerald-500/20" },
                { label: "Total Egresos",   value: fmt(totalEgresos),                    sub: "salidas del período", icon: <TrendingDown size={16} className="text-rose-400" />, ibg: "bg-rose-500/10 border-rose-500/20"       },
              ].map((card) => (
          ...
          {isLoading && (
            <div className="flex flex-col items-center justify-center py-28 gap-4">
              <div className="relative">
                ...
                <Wallet size={22} className="text-blue-400 absolute inset-0 m-auto" />
              </div>
              <p className="text-sm text-slate-400 font-bold animate-pulse">Cargando aperturas de caja...</p>
            </div>
          )}
          ...
          {!isLoading && error && (
            <div className="flex flex-col items-center justify-center py-24 gap-3 text-rose-400">
              ...
              <button onClick={() => fetchAperturas(period, dateFrom, dateTo)}
          ...
          {!isLoading && !error && aperturas.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 gap-3 text-slate-500">
              ...
              <p className="text-sm opacity-60">No hay aperturas de caja para este período.</p>
            </div>
          )}

          {/* ── Kanban board ───────────────────────────────────── */}
          {!isLoading && !error && sedeEntries.length > 0 && (
            <div className="space-y-10">
              {sedeEntries.map(([idSede, { sede, items }], sedeIdx) => {
                const pal  = SEDE_PALETTES[sedeIdx % SEDE_PALETTES.length];
                const sedeTotal  = items.reduce((s, a) => s + Number(a.TotalVentas),  0);
                const sedeEgreso = items.reduce((s, a) => s + Number(a.TotalEgresos), 0);
                const sedeOpen   = items.filter((a) => !a.FechaCierre).length;
                ...
                return (
                  <section key={idSede}>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mb-5">
                      ...
                      <div>
                        <h3 className="text-base font-black text-white">{sede}</h3>
                        <p className="text-[10px] text-slate-500">
                          {items.length} apertura{items.length !== 1 ? "s" : ""}
                    ...
                    {/* Aperturas — each row is a horizontal pipeline of 4 cards */}
                    <div className="space-y-4">
                      {items.map((ap) => {
                        ...
                        return (
                          <div key={ap.IdApertura} className={`rounded-2xl border p-4 ${
                            isOpen
                            ...
                            <button
                              onClick={() => openVentas(ap.IdApertura, ap.IdSede)}
                            ...
                            <button
                              onClick={() => openEgresos(ap.IdApertura, ap.IdSede)}
                            ...
                            <button
                              onClick={() => openCorte(ap.IdApertura, ap.IdSede)}
