...
// Ventas agrupadas por forma de pago (para el treemap y el grid).
export async function GET(request: Request) {
    try {
        ...
        // Se agrupa por IdFormaPago y la etiqueta sale del propio pago (MAX) porque
        // tblPagos guarda ambas columnas; un id sin etiqueta se reporta como OTRO.
        const [rows] = await pool.query(
            `SELECT
                COALESCE(P.IdFormaPago, 0) AS IdFormaPago,
                COALESCE(NULLIF(MAX(P.FormaPago), ''), 'OTRO') AS FormaPago,
                COUNT(*) AS Cantidad,
                COALESCE(SUM(P.Pago), 0) AS Total
             FROM tblPagos P
             INNER JOIN tblProductos PR ON P.IdProducto = PR.IdProducto
             WHERE ${where}
             GROUP BY COALESCE(P.IdFormaPago, 0)
             ORDER BY Total DESC`,
            params
        ) as any[];
        ...
        console.error('Error fetching ventas por forma de pago:', error);
        return NextResponse.json(
            { success: false, message: 'Error al obtener ventas por forma de pago', error: error instanceof Error ? error.message : 'Unknown' },
            { status: 500 }
        );
    }
}
