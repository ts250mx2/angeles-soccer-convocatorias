...
// Ventas agrupadas por tipo de producto (para el treemap y el grid).
    ...
    try {
        ...
        const [rows] = await pool.query(
            `SELECT
             ...
             WHERE ${where}
             GROUP BY PR.IdTipoProducto, TP.TipoProducto
             ORDER BY Total DESC`,
            params
        ) as any[];
        ...
        console.error('Error fetching ventas por tipo:', error);
        return NextResponse.json(
            { success: false, message: 'Error al obtener ventas por tipo', error: error instanceof Error ? error.message : 'Unknown' },
            { status: 500 }
        );
    }
}
