...
import { X, Loader2, AlertCircle, Check, UserRoundPlus, PenLine, ArrowRight, Pencil } from "lucide-react";
import BuscadorIncremental, { type OpcionBuscador } from "@/components/BuscadorIncremental";
import type { IncorporacionRow, JugadorBuscado, OpcionProfesor } from "@/lib/incorporaciones";

/**
 * Captura del formato de incorporación, en el mismo orden del papel: fecha, profesor,
 * jugador, procedencia (que se llena sola), grupo a incorporar, justificación y firma.
 *
 * La procedencia no es un campo capturable: se toma de la categoría del jugador elegido
 * y se muestra en gris para que se vea de dónde viene. El servidor la vuelve a leer de
 * la base al guardar, así que lo que se pinta aquí es informativo.
 */
...
export function NuevaIncorporacionModal({
  temporadaId, temporada, profesores, categorias, autorizante, onClose, onGuardado,
}: {
  temporadaId: number;
  ...
  return (
    <Marco titulo="Formato de incorporación" subtitulo={temporada} icono={<UserRoundPlus size={18} />} onClose={onClose}>
        ...
        {procedencia && grupo && (
          <div className="flex items-end pb-2">
            <p className="flex items-center gap-2 text-xs text-slate-300">
              ...
              <span className="px-2 py-1 rounded bg-blue-600/20 border border-blue-500/40 font-bold text-blue-200">{grupo.etiqueta}</span>
            </p>
          </div>
        )}
