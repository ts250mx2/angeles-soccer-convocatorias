...
/*
 * Los cobros de copas y ligas, compartidos por sus tres rutas: todo (/pagos-copas),
 * solo copas y solo ligas. Es la misma pantalla con distinto corte; el tipo lo dice el
 * producto (IdTipoProducto: 3 liga, 4 copa), asi que filtrar no cuesta una consulta.
 */

import { TIPO_PRODUCTO_COPA, TIPO_PRODUCTO_LIGA } from '@/lib/copas-ligas';
...
import {
  Trophy, Search, ChevronRight, X, CreditCard,
  RefreshCw, BarChart3, TrendingUp, User, FileDown, AlertTriangle, History
} from "lucide-react";
...
export default function PagosCopasPantalla({ tipo }: { tipo?: TipoTorneo }) {
  ...
  const handleExportDetailsPDF = () => {
    ...
    presentarPdf(doc, `Pagos_${selectedProduct.Producto.replace(/\s+/g, '_')}_${selectedCategory.replace(/\s+/g, '_')}.pdf`);
  };

  /* El tipo del cobro lo dice su producto: 3 es liga y 4 es copa (TIPO_PRODUCTO_LIGA
     y TIPO_PRODUCTO_COPA del catálogo). Con la ruta acotada, el otro tipo ni se pinta. */
  const filteredProducts = products.filter(p =>
    (!tipo || (tipo === 'copa'
      ? p.IdTipoProducto === TIPO_PRODUCTO_COPA
      : p.IdTipoProducto === TIPO_PRODUCTO_LIGA)) &&
  ...
  return (
    <DashboardLayout>
      <main className={`overflow-y-auto flex-1 text-white ${acento?.fondo ?? ''}`}>
        {/* Header */}
        <div className={`bg-white/5 backdrop-blur-xl px-6 py-4 flex justify-between items-center sticky top-0 z-20 ${acento ? acento.filoInferior : 'border-b border-white/10'}`}>
          <div className="flex items-center gap-4">
            <div className={`p-2.5 rounded-xl border ${acento ? acento.caja : 'bg-blue-600/20 border-blue-500/20'}`}>
              <Trophy size={20} className={acento ? acento.icono : 'text-blue-400'} />
            </div>
            <div>
              <h1 className="text-xl font-black">
                {tipo === 'copa' ? 'Pagos de Copas' : tipo === 'liga' ? 'Pagos de Ligas' : 'Pagos de Copas y Ligas'}
              </h1>
              <p className={`text-xs ${acento ? acento.icono : 'text-blue-300'}`}>{season ? `Temporada ${season}` : "Gestión de torneos"}</p>
            </div>
          </div>
